...
async function processAsset(asset) {
  ...
  try {
    ...
    Logger.info(
      `processAsset :: Fetched quote for ${asset.name} - ${
        asset.symbol
      } : ${JSON.stringify(snap)}`
    );
    ...
    if (asset.upperThreshold != null && quote.price >= asset.upperThreshold) {
      ...
      asset.lowerThreshold != null &&
      quote.price <= asset.lowerThreshold
    ) {
      alertBoundary = "lower";
    }
    ...
    if (alertBoundary) {
      ...
      if (now - last > COOLDOWN_MS) {
        Logger.info(
          `processAsset :: Sending alert for asset ${asset.name} - ${asset.symbol} at price ${quote.price}`
        );
        ...
        const payload = {
          ...
          threshold:
            alertBoundary === "upper"
              ? asset.upperThreshold
              : asset.lowerThreshold,
        ...
        AlertHistory.create({
          ...
          Logger.error(`AlertHistory create failed - ${err.message}`)
        );
      }
    }
...
export async function runCycle() {
  Logger.info(
    "[Scheduler] Running fetch cycle (manual)",
    new Date().toISOString()
  );
  const assets = await Asset.find();
  Logger.debug(
    `runCycle :: fetched assets to process - ${JSON.stringify(assets)}`
  );
